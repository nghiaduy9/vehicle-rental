    ...
    <div class="container w-50" v-if="cookieCheck">
        ...
        <form @submit.prevent="payment">
            <div class="form-group">
                <input class="form-control" placeholder="Rental Agreement Id" type="number" required v-model="rentalId">
            </div>
            <div class="form-group">
                <input class="form-control" placeholder="Time End" type="date" required v-model="timeEnd">
            </div>
            <div class="form-group">
                <input class="form-control" placeholder="Total Price" type="number" required v-model="totalPrice">
            </div>
            <div class="form-group">
...
<script>
  ...
  async function getRental(rentalId) {
    let check = true;
    let rental;
    let p = await axios.get('http://localhost:3000/api/RentalAgreement/'+rentalId)
      .then(function (res) {
        rental = res.data;
      })
      .catch(function (e) {
    ...
    methods: {
      payment: async function () {
        ...
        if (rental){
          ...
          this.noti = 'Unsuccessful, rental is not existed'
      }
    }
  }
</script>